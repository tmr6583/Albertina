...
| Upsert | aplicar `ON CONFLICT` por chave natural Olist e `tenant_id` |
| Analítico | atualizar `vw_*` automaticamente e `mv_*` de forma seletiva |
...
| Analítico | `mv_dim_contacts` quando houver impacto analítico |
...
| Analítico | `mv_fact_orders` e `mv_fact_order_items` |
...
| Analítico | `mv_dim_products` e `mv_fact_inventory` |
...
| Estratégia | varredura por workflow fiscal, sem mart dedicada nesta fase |
...
| Estratégia | janela por faixa temporal, conforme estratégia financeira |
...
| Estratégia | depende do workflow; parte usa `cooldown` |
...
| Estratégia | leitura por agrupamento com detalhamento posterior |
...
| Estratégia | leitura de lista com detalhamento por separação |
